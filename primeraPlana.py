...
class ImageViewer(QGraphicsView):
    ...
    def create_fixed_text(self):
        ...
        self.title_edit.setGeometry(500, 500, 400, 50)  # Posición fija (x, y, ancho, alto)

        # Campo de texto para el contenido
        self.content_edit = QTextEdit(self)
        self.content_edit.setFont(QFont("Times New Roman", 12))
        ...
        pixmap = QPixmap(ruta_recurso("temporada/Prueba2.gif"))  
        scaled_pixmap = pixmap.scaled(690, 150, Qt.AspectRatioMode.KeepAspectRatio)
        ...
        self.content_edit.textChanged.connect(lambda: self.limit_text(self.content_edit, 500))  # máximo 500 caracteres
    ...
    def adjust_text_position(self, value):
        ...
        self.title_edit.move(700, 470 - scroll_offset)
        self.content_edit.move(670, 520 - scroll_offset)
        self.reporter_edit.move(450, 680 - scroll_offset)
        self.capture_button.move(450, 530 - scroll_offset)
        self.photo_label.move(450, 480 - scroll_offset)
        self.second_image_label.move(450, 740 -scroll_offset)
    ...
    def draw_widgets_on_pdf(self, painter, img_width, img_height, page_width, page_height, img_pos, scroll_value):
        ...
        title_x_scaled = int((title_x - img_pos.x() + 180))  # Ajustar posición X
        title_y_scaled = int((title_y - img_pos.y() - scroll_value + 870))  # Ajustar posición Y

        # Establecer la fuente para el título
        title_font = QFont("Times New Roman", 23)  # Tamaño de fuente para el título
        ...
        content_x_scaled = int((content_x - img_pos.x() + 100))  # Ajustar posición X
        content_y_scaled = int((content_y - img_pos.y() - scroll_value + 940))  # Ajustar posición Y

        # Establecer la fuente para el contenido
        content_font = QFont("Times New Roman", 12)  # Tamaño de fuente para el contenido
        ...
        # En el método draw_widgets_on_pdf, modificar la parte del contenido:
        max_content_height = 600  # Altura máxima en píxeles para el contenido
        current_y = content_y_scaled
        line_height = fm_content.height()
        line_spacing = 35  # Espacio entre líneas reducido

        for i, line in enumerate(wrapped_lines):
            # Verificar si hemos alcanzado el límite vertical
            if current_y + line_height > content_y_scaled + max_content_height:
                break  # Detener el dibujo si excede el límite
        ...
        reporter_x_scaled = int((reporter_x - img_pos.x() - 495))  # Ajustar posición X
        reporter_y_scaled = int((reporter_y - img_pos.y() - scroll_value + 1220))  # Ajustar posición Y

        # Establecer la fuente para el nombre del reportero
        reporter_font = QFont("Times New Roman", 16)  # Tamaño de fuente para el reportero
        ...
        # Dibujar la segunda imagen (QLabel)
        if self.second_image_label.pixmap():
            ...
            second_img_x_scaled = int((second_img_x - img_pos.x() - 300 ))  # Ajustar posición X
            second_img_y_scaled = int((second_img_y - img_pos.y() - scroll_value + 1350))  # Ajustar posición Y
            second_img_w_scaled = int(second_img_w * 2.8)  # Ajustar ancho
            second_img_h_scaled = int(second_img_h * 2.6)  # Ajustar alto
        ...
        # Dibujar la foto capturada (QLabel)
        if self.photo_label.pixmap():
            photo_x, photo_y, photo_w, photo_h = self.photo_label.geometry().getRect()
            photo_x_scaled = int((photo_x - img_pos.x() - 300))  # Ajustar posición X
            photo_y_scaled = int((photo_y - img_pos.y() - scroll_value + 890))  # Ajustar posición Y
            photo_w_scaled = int(photo_w * 2.5)  # Ajustar ancho
            photo_h_scaled = int(photo_h * 2.1)  # Ajustar alto
...
if __name__ == "__main__":
    ...
    window = ImageViewer(ruta_recurso("backgrounds/PrimeraPlana.png"))  # Cambia por la ruta real de tu imagen        
